...
static esp_err_t tft_create_mutex( void ) {

	tft_mutex = xSemaphoreCreateMutex();
    if( !tft_mutex )
    {
        ESP_LOGE(TAG, "Could not create device tft_mutex");
        return ESP_FAIL;
    }
...
esp_err_t tft_take_mutex( void ) {

    if( !xSemaphoreTake( tft_mutex, pdMS_TO_TICKS( TFT_MUTEX_TIMEOUT )) ) {

        ESP_LOGE(TAG, "Could not take tft_mutex" );
...
esp_err_t tft_give_mutex( void ) {

    if( !xSemaphoreGive( tft_mutex ) ) {
        ESP_LOGE(TAG, "Could not give tft_mutex" );
        return ESP_FAIL;
    }
...
void delayMS(int ms) {
	...
	ESP_LOGD(TAG, "ms=%d _ms=%d portTICK_PERIOD_MS=%d xTicksToDelay=%d",ms,_ms,(int)portTICK_PERIOD_MS,(int)xTicksToDelay);
...
void lcdInit(TFT_t * dev, int width, int height, int offsetx, int offsety)
{
	...
	spi_master_write_command(dev, 0xC0);   /* LCM Control, XOR: BGR, MX, MH */
	...
	spi_master_write_command(dev, 0xB2); /* Porch Setting */
